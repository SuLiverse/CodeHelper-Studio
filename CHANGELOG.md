...
- 桌宠 ZIP 导入在 Windows 与非 Windows 都先校验条目路径，解压后再拒绝符号链接和目录逃逸。
- `spritesheetPath` 拒绝盘符相对路径、绝对路径和 `..` 穿越。
- 桌宠在线安装只允许 `codex-pet.org` HTTPS 资源，并拒绝重定向与 URL 凭据。
- 遗留 Markdown 协议相对链接不再渲染为可点击链接。
- 设置页识别升级前明文 API Key，提示保存一次以重加密；保存路径继续 fail-closed。
- 恢复层导出在保存对话框省略后缀时强制补上 `.json`。
- 将运行时 `undici` 升级到 `6.28.0`，并用 overrides 钉住 `nanoid@3.3.18` 与 jsdom 的 `undici@7.29.0`；完整 `npm audit` 为 0。
